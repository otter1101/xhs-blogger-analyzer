<div align="center">

# 博主蒸馏器

> "你研究了 10 个小红书对标博主。你能说清楚任何一个的爆款打法吗？"

你翻了 50 条笔记，还是说不出 TA 为什么爆？<br>
你总结出来了，却只有"风格活泼、选题接地气"——然后呢？<br>
你换了个对标博主，一切又得从头研究？<br>
你用了 AI，但它写出来的还是很普通——它根本不知道你想学谁。<br>
你自己也发了三十条笔记，但每次创作还是凭感觉从零开始？<br>

**把任何博主（包括你自己）的爆款打法，从真实笔记里蒸馏出来，装进你的 AI。**

输入博主账号，30 分钟后拿走 TA 的爆款打法——  
装进你的 AI，变成你永久的内容教练。

[![Python 3.8+](https://img.shields.io/badge/Python-3.8+-blue.svg)](https://www.python.org/downloads/)
[![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](LICENSE)

[30 秒上手](#30-秒上手) · [产出物](#产出物) · [分析模式](#分析模式) · [架构设计](#架构设计)

</div>

---

## 它做了什么

把「手工翻笔记 → 凭感觉总结」变成「一句话 → 自动蒸馏 → 拿结果」。

```
拆解/蒸馏小红书博主 XX
```

AI 自动完成全流程：

```
扫码登录 → 爬取笔记 → 统计分析 → 认知层提取 → 蒸馏 → 出报告
```

你拿到的不是一堆原始数据，是两样东西：

| 产出物 | 给谁 | 干什么 |
|--------|------|--------|
| **创作指南 Skill 文件夹** | 给你的 AI | 装进 AI 工具后，说"用 XX 风格写一篇笔记"，AI 立刻知道怎么写 |
| **HTML 蒸馏报告** | 给你看 | 浏览器打开，快速读完这个博主的人设、策略和爆款规律 |

> **和竞品的区别**：市面上的蒸馏类 Skill（同事.skill / 前任.skill / 女娲.skill）都需要你手动提供数据。博主蒸馏器是目前唯一**输入端全自动**的蒸馏类 Skill：你只需要说一个博主的名字。

---

## 产出物

### 创作指南 Skill（给 AI 用）

生成一个可安装的 Skill 文件夹。把它放到你的 AI 工具里：

```
.claude/skills/XX博主_创作指南.skill/
└── SKILL.md
```

安装后，在新对话里直接说：

```
用XX博主的风格，写一篇关于 AI 工具推荐的小红书笔记
```

AI 按照蒸馏出的公式、语气和结构来写——不是泛泛的"活泼风格"，是从真实笔记里提炼的具体规则。

**三层蒸馏结构：**

| 层级 | 回答什么 | 装进 Skill 的内容举例 |
|------|---------|----------------------|
| **认知层** | TA 怎么想？ | "TA 相信'反共识才有传播力'，80% 爆款都用了反常识观点开头" |
| **策略层** | TA 怎么运营？ | "每周三晚 8 点发，藏赞比 0.85 说明粉丝当教程收藏" |
| **内容层** | TA 怎么写？ | "35% 数字标题、25% 反问标题，开头永远是个人故事" |

8 大章节：使用说明 → 认知层 → 策略层 → 内容层 → 创作禁区 → 对比示例 → 选题灵感 → 局限性自检

### HTML 蒸馏报告（给你看）

单文件 HTML，浏览器打开即可阅读。

| # | 模块 | 内容 |
|---|------|------|
| 1 | 一眼看清 | 粉丝/获赞/爆款率/底层公式，一张卡片看完 |
| 2 | 人设拆解 | TA 是谁、代表什么、粉丝为什么追 TA |
| 3 | 认知层 | 核心信念 × 观点张力 × 价值立场 |
| 4 | 策略层 | 系列规划 × 蹭热点方式 × 运营节奏 |
| 5 | TOP10 爆款 | 逐条拆解：为什么爆？能学什么？ |
| 6 | 内容公式 | 11 种标题公式 × 开头模板 × CTA 策略 |
| 7 | 选题灵感 | TOP15 选题方向，按难度 × 潜力排序 |
| 8 | 数据面板 | 均赞/均藏/藏赞比/视频 vs 图文/发布频率 |
| 9 | 发展趋势 | 早期 vs 近期变化，转型路径（附置信度标注）|
| 10 | 核心结论 | 三句话总结 + 行动建议 |

---

## 分析模式

| 模式 | 你想做什么 | 产出物 |
|------|-----------|--------|
| **A — 拆解对标博主** | 学习 TA 的打法 | HTML 报告 + `{博主名}_创作指南.skill/` |
| **B — 蒸馏自己的账号** | 打造属于自己的创作工作流 | HTML 报告 + `{用户名}_创作基因.skill/` |
| **C — 对标 + 借鉴** | 对比 TA 和我的差距 | 对比 HTML 报告 *(v2.1 上线)* |

笔记数量三档可选：30 条（快速）/ 50 条（推荐）/ 80 条（深度）

---

## 30 秒上手

```bash
git clone https://github.com/otter1101/blogger-distiller.git
cd blogger-distiller
python install.py
```

在你的 AI 编程助手里说：

```
拆解博主 XX
```

系统会问你选模式（A 或 B）和笔记数量，之后全自动。第一次需要扫码登录小红书，之后不用。

<details>
<summary>命令行用法（进阶）</summary>

```bash
python run.py "XX博主"
python run.py "XX博主" --skip-env
```

</details>

---

## 架构设计

**「脚本保下限，AI 冲上限」**

| 角色 | 占比 | 负责什么 |
|------|------|---------|
| **脚本** | 30% | 爬数据、算统计、识别标题模式、提取 CTA、计算藏赞比 |
| **AI** | 70% | 提炼信念、发现认知张力、写人设拆解、做因果分析、给个性化建议 |

**核心特性：**

- **零配置** — 自动下载依赖、启动服务，开箱即用
- **扫码登录** — 首次扫码后自动保存登录态，后续无需重复操作
- **低并发爬取** — 每条间隔 3 秒，最大程度降低封号风险
- **跨领域通用** — 不预设内容类型，基于笔记实际内容动态聚类，适用任何赛道
- **断点恢复** — 每 10 条自动存盘，断网/中断不丢数据

---

## 项目结构

```
blogger-distiller/
├── SKILL.md                  # Skill 定义（AI 编程助手读这个）
├── run.py                    # 一键运行入口
├── install.py                # 自动安装脚本
├── scripts/
│   ├── check_env.py          # Phase 0: 环境准备
│   ├── crawl_blogger.py      # Phase 1: 数据采集（主页+搜索+详情+断点）
│   ├── analyze.py            # Phase 2: 数据分析（聚类+标签+TOP10+认知层）
│   ├── deep_analyze.py       # Phase 3: 数据底稿 + AI 蒸馏任务生成
│   ├── verify.py             # Phase 4: 数据校验
│   └── utils/
│       ├── mcp_client.py     # MCP HTTP 客户端（纯 urllib）
│       └── common.py         # 共用工具函数
└── references/
    └── 产出物质量标杆.md
```

---

## 版本历程

| 版本 | 里程碑 |
|------|--------|
| v0.1–v1.0 | SKILL 设计 → 全 Phase 脚本 → 通用化重构 → 全自动环境准备 → 输入端稳定化 |
| v1.7 | 认知层提取（观点句/思维模式/价值词） |
| v1.8 | 产出物改版（HTML 报告 + Skill 文件夹） |
| v1.9 | 全流程切换新范式 + 爬取 Bug 修复 |
| **v2.0** | **目标：模式 A 全流程验收收口** |

---

## License

MIT License 

<div align="center">

**让 AI 帮你蒸馏出优秀者的思维方式，变成你自己的创作武器**

有任何问题，欢迎进群讨论～
![da597296e12cb0287e8a9112d12942b0](https://github.com/user-attachments/assets/d2ee0a37-8971-4750-90ee-abfa47fac92c)

![008863a2cfa1276a502683b1faf908db](https://github.com/user-attachments/assets/d065d0be-9926-4e4f-a14f-6ead7621cf72)

</div>
